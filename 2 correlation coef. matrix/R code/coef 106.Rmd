...
//移除分數最低0.1%者 
```{r}
df1 = read.csv("../organized data/106 各科之分數分布.csv")
...
```{r}
df2 = read.csv("../organized data/106 各組合之分數分布.csv")
grade_array = df2[,2]
num_of_people_array = df2[,3]
each_stud_score <- vector("double", length = 46000)   #temp, scores of each student
each_group_var<- vector("double", length = 55)  #variance of each group
each_group_mean<- vector("double", length = 55)     #mean of each group

for(i in 0:54){  #total 55 groups 0-54
...
## 科目組合矩陣[n,45]
```{r}
df3 = read.csv("../organized data/106 各組合之採計科目.csv")
...
# 科目組合矩陣 x:各組合所包含的變異數(權重=1)  y:共55組合
mtrx <- matrix(0,nrow=55,ncol=45) 
...
var123plus <- vector("double", length = 55)   #sum of subjects' var in each group
...
columns '410' and associated rows
```{r}
ind1 = mtrx_orig[,'410'] == 0
y2 = y1[ind1]

mtrx2 = data.frame(mtrx_orig[ind1,])
mtrx2 = mtrx2[,which(colSums(abs(mtrx2))!=0)]  #remove column if the whole column are all 0
mtrx2 = mtrx2[,!colnames(mtrx2) %in% 'X410']    #remove column 'X410' since only one group contains both subject4 & subject10
