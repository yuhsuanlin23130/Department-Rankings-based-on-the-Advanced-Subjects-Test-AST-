...
//移除分數最低0.1%者 
```{r}
df1 = read.csv("../organized data/105 各科之分數分布.csv")
...
```{r}
df2 = read.csv("../organized data/105 各組合之分數分布.csv")
grade_array = df2[,2]
num_of_people_array = df2[,3]
each_stud_score <- vector("double", length = 49000)    #temp, scores of each student
each_group_var<- vector("double", length = 51)  #variance of each group
each_group_mean<- vector("double", length = 51)     #mean of each group

for(i in 0:50){  #total 51 groups 0-50
  cnt=1
  a =1+i*100
  b=100+i*100
  for(j in a:b)
...
## 科目組合矩陣[n,45]
```{r}
df3 = read.csv("../organized data/105 各組合之採計科目.csv")
...
# 科目組合矩陣 x:各組合所包含的變異數(權重=1)  y:共51組合
mtrx <- matrix(0,nrow=51,ncol=45) 
...
var123plus <- vector("double", length = 51)   #sum of subjects' var in each group
